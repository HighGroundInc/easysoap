(function() {
    ...
    function _getRequestHead(callParams, namespaces, opts) {
        ...
        return new Promise(function(resolve, reject) {
            ...
            var headerParts = [];
            _.each(callParams.header, function(headerItem, index) {
                for (var property in headerItem) {
                    var item = '<cns' + index + ':' + property + '>';
                    headerParts.push(item); 
                    if (headerItem[property] != null && _.isObject(headerItem[property])) {
                        if (Object.keys(headerItem[property]).length > 0) {
                            var childElement = headerItem[property];
                            Object.keys(childElement).forEach(function(key) {
                                if (key) {
                                    var item = '<cns' + index + ':' + key + '>';
                                    item += childElement[key];
                                    item += '</cns' + index + ':' + key + '>';
                                    headerParts.push(item); 
                                }
                            });
        ...
        var requestItems = function(cParams, mParams, namespace) {
            ...
            _.each(cParams, function(cParam, cParamName) {
                ...
                if (cParam === null) {
                    ...
                    if (_.isObject(cParam) || _.isArray(cParam)) {
...
 
                                if (_.isObject(cParamValue)) {
                                    ...
                                    Object.keys(cParamValue).forEach(function(key) {
                                        if (key) {
                                            //item += '<' + namespace_string + key + '>';
                                            item += cParamValue[key];
                                            //item += '</' + namespace_string + key + '>';
                                        }
                                    });
